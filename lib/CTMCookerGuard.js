...
class CTMGenericCookerGuard extends ZigBeeDevice {
    ...
    async onNodeInit({zclNode}) {
        ...
        if(this.hasCapability('button.refresh') === false){
            ...
            this.setCapabilityOptions('button.refresh', {
              ...
              title: { "en": "Refresh", "no": "Oppdater" },
              desc: { "en": "Send a request to the mKomfy for updated information", "no": "Send en forespørsel til mKomfy på oppdatert informasjon. Husk og trykk på Sensoren først så den er våken" }
        ...
        if(this.isFirstInit()){
            try{
                ...
                if(this.print_log === 1)this.log("Error! configureAttributeReporting", error);
            ...
            try {
                ...
                if(this.print_log === 1)this.log('readattribute alarm_status: ', this.readattribute.alarm_status );
                if(this.print_log === 1)this.log('readattribute active: ', this.readattribute.active );
                if(this.print_log === 1)this.log('readattribute change_battery: ', this.readattribute.change_battery );
                ...
                if(this.readattribute.active === 0){
                    this.setCapabilityValue('cooking', false).catch(err => { this.error(err);});
                    
                } else if(this.readattribute.active === 1){
                    this.setCapabilityValue('cooking', true).catch(err => { this.error(err);});
                }

                if(this.readattribute.change_battery === 0){
                    this.setCapabilityValue('alarm_battery', false).catch(err => { this.error(err);});
                    
                } else {
                    this.setCapabilityValue('alarm_battery', true).catch(err => { this.error(err);});
                }
        ...
        **********************************************************************************/ 
        ...
        zclNode.endpoints[1].clusters['CookerGuard'].on('attr.alarm_status', (attr_value) => {
            try {
                ...
                if(this.print_log === 1)this.log('attr.alarm_status: ', attr_value);
        ...
        **********************************************************************************/ 
        ...
        zclNode.endpoints[1].clusters['CookerGuard'].on('attr.active', (attr_value) => {
            try {
                ...
                this.setAvailable().catch(err => { this.error(err);});
                if(this.print_log === 1)this.log('attr.active: ', attr_value);
                if(attr_value === 0){
        ...
        **********************************************************************************/ 
        ...
        zclNode.endpoints[1].clusters['CookerGuard'].on('attr.alarm_battery', (attr_value) => {
            try {
                ...
                this.setAvailable().catch(err => { this.error(err);});
                if(this.print_log === 1)this.log('attr.alarm_battery: ', attr_value);
                if(attr_value === 0){
        ...
        **********************************************************************************/ 
        ...
        this.registerCapabilityListener('button.refresh', async () => {
			...
			try {
				...
				throw new Error('Something went wrong. You need to keep the sensor awake by pressing the button on the switch itself before making this request.');
			}
    ...
    async alarm_decode(alarm){
        try {
            if(this.print_log === 1)this.log('Alarm_Decode', alarm);
            if (this.hasCapability('heartbeat')){
                this.setCapabilityValue('heartbeat', false).catch(err => { this.error(err);});
                this.setCapabilityValue('heartbeat', true).catch(err => { this.error(err);});
            }
    ...
    async update_settings(dip_sw){
        try {
            ...
            if(this.print_log === 1)this.log("DIP_SW Settings:", dip_sw);
